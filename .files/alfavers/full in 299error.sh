...
install_opt_packages() {
    ...
    if ! command -v whiptail >/dev/null 2>&1; then
        echo -e "\033[1;33mInstalling whiptail...\033[0m"
        opkg update && opkg install whiptail
        if [ $? -ne 0 ]; then
            echo -e "\033[0;31mFailed to install whiptail. Exiting...\033[0m"
            return 1
        fi
    fi

    # Package list from user's provided code
    ...
    if [ $? -ne 0 ]; then
        echo -e "\033[1;33mInstallation canceled by user.\033[0m"
        return 0
    fi

    # Update opkg
    echo -e "\033[0;32mUpdating package lists...\033[0m"
    opkg update
    ...
    for package in $selected_packages; do
        ...
        if [ $? -eq 0 ]; then
            echo -e "\033[0;32m$package_name installed successfully ✅\033[0m"
        else
    ...
    if opkg list-installed | grep -q "^sing-box "; then
        echo -e "\033[1;33mConfiguring SingBoX shunt...\033[0m"
        ...
        echo -e "\033[0;32mSingBoX shunt configured successfully ✅ OK\033[0m"
    else
        echo -e "\033[0;31mSingBoX is not installed. Skipping shunt configuration.\033[0m"
    fi
...
function run_script()
    ...
    elseif action == "set_wifi_config" then
        cmd = cmd .. " '" .. (luci.http.formvalue("ssid") or "") .. "' '" .. (luci.http.formvalue("key") or "") .. "'"
