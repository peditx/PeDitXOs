...
# PeDitXOS Tools - Simplified Installer Script v44 (Patched)
# This version adds custom profile and bash configuration.
...
install_warp() {
    ...
    cd /tmp
    rm -f warpplus.sh
    wget https://raw.githubusercontent.com/peditx/iranIPS/refs/heads/main/.files/extras/warpplus.sh -O warpplus.sh
    chmod 777 warpplus.sh
    sh warpplus.sh
...
# Create the View file (v44)
cat > /usr/lib/lua/luci/view/peditxos/main.htm << 'EOF'
<%# LuCI - Lua Configuration Interface v44 %>
<%+header%>
<style>
    :root {
    ...
    .peditx-tabs { display: flex; border-bottom: 2px solid var(--peditx-border); margin-bottom: 15px; flex-wrap: wrap; }
    .peditx-tab-link { background-color: var(--peditx-card-bg); border: none; outline: none; cursor: pointer; padding: 14px 16px; transition: 0.3s; font-size: 17px; border-bottom: 3px solid transparent; color: var(--peditx-text-color); border-top-left-radius: 8px; border-top-right-radius: 8px; margin-right: 5px; }
    .peditx-tab-link:hover { background-color: var(--peditx-hover-bg); }
    .peditx-tab-link.active { border-bottom: 3px solid var(--peditx-primary); background-color: var(--peditx-primary); color: white; }
    ...
    #execute-button { font-size: 18px; padding: 15px 40px; color: white; background: linear-gradient(135deg, #00b5e2, #008eb2); border: none; border-radius: 50px; box-shadow: 0 4px 15px rgba(0, 181, 226, 0.4); transition: background 0.3s ease, transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer; }
    #execute-button:hover { background: linear-gradient(135deg, #008eb2, #006b8c); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0, 181, 226, 0.5); }
    #execute-button:disabled { background: #555; cursor: not-allowed; box-shadow: none; transform: none; }
