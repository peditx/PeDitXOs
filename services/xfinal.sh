...
chmod +x /etc/profile.d/30-sysinfo.sh > /dev/null 2>&1
chmod +x /etc/profile.d/sys_bashrc.sh > /dev/null 2>&1
# --- End of Configuration ---

# --- Step 1: Initial system configuration ---
uci set system.@system[0].zonename='Asia/Tehran' > /dev/null 2>&1
uci set system.@system[0].hostname='PeDitXOS' > /dev/null 2>&1
...
opkg install curl luci-compat screen sshpass procps-ng-pkill luci-app-ttyd coreutils coreutils-base64 coreutils-nohup > /dev/null 2>&1
...
install_theme() {
    ...
    if ! opkg install "$filename" > /dev/null 2>&1; then
        rm -f "$filename"
        return 1
    fi
...
# --- Step 2: Creating/Updating the LuCI application ---
mkdir -p /usr/lib/lua/luci/controller /usr/lib/lua/luci/model/cbi /usr/lib/lua/luci/view/peditxos

# --- Create directories and files for the new Store page by downloading them ---
...
exit 0
EOF
chmod +x /usr/bin/peditx_runner.sh > /dev/null 2>&1
...
EOF
chmod 644 /usr/lib/lua/luci/controller/peditxos.lua > /dev/null 2>&1
...
# --- Finalization ---
echo ""
echo ">>> PeDitXOS Tools installed successfully! Please reboot or re-login to see all changes."
